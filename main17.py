...
from fastapi import FastAPI, Request, HTTPException, Query, BackgroundTasks, UploadFile, File, Form, WebSocket, WebSocketDisconnect, Response
...
async def query_internet_via_groq(
    ...
    try:
        ...
        sources = []
        executed_tools = getattr(completion.choices[0].message, "executed_tools", []) or []
...
async def generate_seed_keywords(query: str) -> list:
    ...
    if len(seed_keywords) != 3:
        ...
        logger.warning(f"Expected 3 seed keywords, but got {len(seed_keywords)}. Using what was returned.")
...
async def generate_caption(query: str, seed_keywords: list, trending_hashtags: list, seo_keywords: list) -> str:
...
async def generate_text_embedding(text: str | None) -> list:
    if not text:
        logging.warning("generate_text_embedding called with empty or None text; returning empty embedding.")
        return []
    try:
...
async def content_for_website(content: str) -> str:
    ...
    try:
        ...
        response = await asyncio.to_thread(
            client.chat.completions.create,
            messages=[{"role": "system", "content": "You are a content analysis expert."},
...
async def detailed_explanation(content: str) -> str:
    ...
    try:
        ...
        response = await asyncio.to_thread(
            client.chat.completions.create,
            messages=[{"role": "system", "content": "You are an expert analysis assistant."},
                      {"role": "user", "content": prompt}],
            model="llama3-70b-8192",
            max_tokens=700,
            temperature=0.8
...
async def classify_prompt(prompt: str) -> str:
    try:
        ...
        response = await asyncio.to_thread(
            client.chat.completions.create,
            messages=[{"role": "system", "content": "Determine if this query requires real time research. Respond with 'research' or 'no research'."},
                      {"role": "user", "content": prompt}],
            model="llama3-70b-8192"
        )
...
async def retrieve_multimodal_context(query: str, session_id: str, filenames: list[str] = None) -> tuple[str, set]:
    try:
        ...
        if embedding and doc_index.ntotal > 0:
            ...
            for idx in indices[0]:
                ...
                if meta and (meta.get("session_id") == session_id or meta.get("session_id") != session_id):
                    if filenames and meta.get("filename") not in filenames:
                        continue
                    filename = meta["filename"]
                    if filename not in file_chunks:
                        file_chunks[filename] = []
                    file_chunks[filename].append((idx, meta, distances[0][indices[0].tolist().index(idx)]))
...
async def efficient_summarize(previous_summary: str, new_messages: list, user_id: str, max_summary_length: int = 500) -> str:
    ...
    try:
        ...
        response = await asyncio.to_thread(
            client.chat.completions.create,
            messages=[{"role": "system", "content": "You are an AI that creates personalized conversation summaries."},
    ...
    try:
        ...
        if mem_entry:
            ...
            if user_id in user_memory_map:
                doc_index.remove_ids(np.array([user_memory_map[user_id]], dtype="int64"))
...
async def schedule_notification(user_id: str, message: str, scheduled_time: datetime.datetime, notif_type: str = "general"):
    notif = {
        "user_id": user_id,
        "message": message,
    ...
    logging.info(f"Notification scheduled for user {user_id} at {scheduled_time} with type '{notif_type}'.")
    ...
    while True:
        ...
        async for notif in cursor:
            ...
            if not user or "push_subscription" not in user:
                logging.warning(f"No subscription found for user {user_id}. Marking notification as skipped.")
                await notifications_collection.update_one(
                    {"_id": notif["_id"]},
                    {"$set": {"sent": True, "sent_at": datetime.datetime.now(datetime.timezone.utc), "status": "skipped_no_subscription"}}
                )
                continue
    ...
    while True:
        async for user in users_collection.find():
            ...
            if 8 * 60 + 55 <= (now_local.hour * 60 + now_local.minute) <= 9 * 60 + 5:
                checkin_time_utc = user_tz.localize(checkin_time_local).astimezone(pytz.UTC)
                ...
                if existing:
                    continue
                goals_cursor = goals_collection.find({"user_id": user_id, "status": {"$in": ["active", "in progress"]}})
                active_goals = []
                ...
                await schedule_notification(user_id, message, checkin_time_utc, notif_type="daily_checkin")
                logging.info(f"Scheduling daily check-in for user {user_id} at {checkin_time_utc}")
        await asyncio.sleep(60)
        ...
        async for user in users_collection.find():
            ...
            for checkin in checkin_times:
                ...
                if abs((now_local - target).total_seconds()) <= 15 * 60:
                    ...
                    await schedule_notification(user_id, message, target_utc, notif_type=checkin["type"])
                    logging.info(f"Scheduling proactive check-in for user {user_id} at {target_utc} with type '{checkin['type']}'")
        await asyncio.sleep(60)
...
async def schedule_immediate_reminder(user_id: str, reminder_text: str):
    ...
    await schedule_notification(user_id, f"Reminder: {reminder_text}", scheduled_time_utc, notif_type="reminder")
...
async def rate_limited_groq_call(*args, **kwargs):
    return await asyncio.to_thread(deepsearch_client.chat.completions.create, *args, **kwargs)
...
async def generate_keywords(clarified_query: str) -> List[str]:
    current_date = get_current_datetime()
    prompt = (
        "Todays date is " + current_date + ". Act as a expert web Browse agent and give 1 search queries from the following query as a JSON array of strings, "
        "e.g. [\"term1\",]:\n\n"
        f"{clarified_query}"
    )
    try:
...
async def deep_search(query_data: Dict[str, str], websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json({"step": "clarified_query", "message": f"Clarified response: {clarifyd_response}"})

        # Generate keywords
        keywords = await generate_keywords(clarified_query)
        understanding = await understanding_query(clarified_query)
        await websocket.send_json({"step": "keywords_generated", "message": f"Processing request: {understanding}"})

        all_responses = []
        all_sources = []
        for keyword in keywords:
            response, sources = await query_internet_via_groq(f"Provide information on {keyword}", return_sources=True)
            if response and response != "Error accessing internet information.":
                ...
                await websocket.send_json({
                    ...
                    "sources": [source["url"] for source in sources]  # Modified to send only URLs
                })

        if not all_responses:
            await websocket.send_json({"step": "no_content", "message": "No information retrieved."})
            return
        ...
        try:
            ...
            unique_sources = [source["url"] for source in {source["url"]: source for source in all_sources}.values()]
            await websocket.send_json({
                "step": "final_answer",
            ...
            await websocket.send_json({"step": "error", "message": "Error generating final answer."})
...
async def upload_file(
    ...
    for file in files:
        try:
            ...
            if file.content_type in allowed_text_types or ext in allowed_code_extensions:
                if ext in allowed_code_extensions:
                    ...
                    if not code_segments:
                        responses.append({"filename": file.filename, "success": False, "message": "Code segmentation failed"})
                        continue
                    ...
                    responses.append({"filename": file.filename, "success": True, "segments": len(code_segments)})
                    logging.info(f"Code file '{file.filename}' segmented into {len(code_segments)} segments for user {user_id} in session {session_id}")
                else:
                    ...
                    if not extracted_text:
                        responses.append({"filename": file.filename, "success": False, "message": "Parsing Failed"})
                        continue
                    ...
                    responses.append({"filename": file.filename, "success": True, "chunks": len(chunks)})
                    logging.info(f"Document file '{file.filename}' uploaded with {len(chunks)} chunks for user {user_id} in session {session_id}")
            else:
                responses.append({"filename": file.filename, "success": False, "message": "Format not allowed"})
        except Exception as e:
            logging.error(f"Error processing file '{file.filename}': {e}", exc_info=True)
            responses.append({"filename": file.filename, "success": False, "message": "File Processing Failed"})
    ...
    try:
        ...
        if url_match:
            ...
            if "youtube.com" in url or "youtu.be" in url:
                external_content = await query_internet_via_groq(f"Summarize the content of the YouTube video at {url}")
                external_content = await detailed_explanation(external_content)
            else:
                external_content = await query_internet_via_groq(f"Summarize the content of the webpage at {url}")
        ...
        if chat_entry:
            ...
            if past_messages:
                ...
                if past_embeddings:
                    logging.info(f"Found {len(past_embeddings)} valid embeddings in chat history.")
                    ...
                    if current_embedding and len(current_embedding) == 768:
                        distances = np.linalg.norm(past_embeddings - np.array(current_embedding), axis=1)
                        n = len(past_messages)
                        ...
                        logging.warning("Current message embedding is invalid; using recent messages only.")
                        chat_history = filter_think_messages(past_messages[-2:]) if len(past_messages) >= 2 else filter_think_messages(past_messages)
                else:
        ...
        generate_api_keys = [os.getenv("GROQ_API_KEY_GENERATE_1"), os.getenv("GROQ_API_KEY_GENERATE_2"), os.getenv("GROQ_API_KEY_GENERATE_3")]
        ...
        async def generate_stream():
            ...
            for goal_id_str, task_desc in task_add_matches:
                ...
                result = await goals_collection.update_one(
                    ...
                    {"$push": {"tasks": new_task}, "$set": {"updated_at": datetime.datetime.now(datetime.timezone.utc)}}
                )
                if result.modified_count > 0:
            ...
            for tid, new_desc in task_modify_matches:
                result = await goals_collection.update_one(
                    {"user_id": user_id, "tasks.task_id": tid},
                    {"$set": {"tasks.$.title": new_desc, "tasks.$.updated_at": datetime.datetime.now(datetime.timezone.utc)}}
                )
                if result.modified_count > 0:
            ...
            for tid in task_start_matches:
                result = await goals_collection.update_one(
                    {"user_id": user_id, "tasks.task_id": tid},
                    {"$set": {"tasks.$.status": "in progress", "tasks.$.updated_at": datetime.datetime.now(datetime.timezone.utc)}}
                )
                if result.modified_count > 0:
            ...
            for tid in task_complete_matches:
                result = await goals_collection.update_one(
                    {"user_id": user_id, "tasks.task_id": tid},
                    {"$set": {"tasks.$.status": "completed", "tasks.$.updated_at": datetime.datetime.now(datetime.timezone.utc)}}
                )
                if result.modified_count > 0:
            ...
            for tid, deadline_str in task_deadline_matches:
                try:
                    ...
                    logging.error(f"Invalid deadline format for task {tid}: {deadline_str}")
                    continue
                user_info = await users_collection.find_one({"user_id": user_id})
                tz_name = user_info.get("time_zone", "UTC") if user_info else "UTC"
                try:
                ...
                result = await goals_collection.update_one(
                    {"user_id": user_id, "tasks.task_id": tid},
                    {"$set": {"tasks.$.deadline": deadline_str, "tasks.$.updated_at": datetime.datetime.now(datetime.timezone.utc)}}
                )
                if result.modified_count > 0:
                    logging.info(f"Set deadline for Task {tid} to {deadline_str}")
                    await schedule_notification(user_id, f"Reminder: Task {tid} is due on {deadline_str}", reminder_time_utc, notif_type="deadline_reminder")
                else:
            ...
            for tid, progress_desc in task_progress_matches:
                progress_entry = {"timestamp": datetime.datetime.now(datetime.timezone.utc), "description": progress_desc}
                result = await goals_collection.update_one(
                    {"user_id": user_id, "tasks.task_id": tid},
                    {"$push": {"tasks.$.progress": progress_entry}, "$set": {"tasks.$.updated_at": datetime.datetime.now(datetime.timezone.utc)}}
                )
                if result.modified_count > 0:
            ...
            cursor = uploads_collection.find({"user_id": user_id, "session_id": session_id, "query_count": {"$gte": 15}})
            ...
            for filename in documents_to_remove:
                ...
                indices_to_remove = [idx for idx, m in file_doc_memory_map.items() if m["filename"] == filename and m["session_id"] == session_id]
                if indices_to_remove:
                    doc_index.remove_ids(np.array(indices_to_remove, dtype="int64"))
                    for idx in indices_to_remove:
                        del file_doc_memory_map[idx]
                code_indices_to_remove = [idx for idx, m in code_memory_map.items() if m["filename"] == filename and m["session_id"] == session_id]
...
async def recommended_content_endpoint(user_id: str = Query(..., description="Unique identifier for the user")):
    try:
        mem_entry = await memory_collection.find_one({"user_id": user_id})
        if not mem_entry or "summary" not in mem_entry:
            raise HTTPException(status_code=404, detail="User summary not found.")
        ...
        for subquery in subqueries:
            response = await query_internet_via_groq(f"Provide a brief summary of {subquery}")
            if response and response != "Error accessing internet information.":
        ...
        logger.info(f"Returning {len(recommended_content)} content items for user {user_id}")
        return {"recommended_content": recommended_content}
    except HTTPException as he:
        raise he
    except Exception as e:
        logger.error(f"Error in /recommended-content for user {user_id}: {e}", exc_info=True)
...
async def send_otp(request: OTPRequest):
    email = request.email
    otp = generate_otp()
    await otp_collection.insert_one({"email": email, "otp": otp, "created_at": datetime.datetime.now(datetime.timezone.utc)})
...
async def visual_synthesize_result(main_query: str, contents: list, max_context: int = 4000) -> str:
...
async def visualization_process(main_query: str, websocket: WebSocket):
    try:
        await websocket.send_json({"step": "start", "message": "Starting visualization research..."})
        
        await websocket.send_json({"step": "generating_subqueries", "message": "Generating subqueries..."})
        ...
        await websocket.send_json({"step": "synthesizing", "message": "Synthesizing research result..."})
        ...
        await websocket.send_json({"step": "generating_html", "message": "Generating HTML visualization..."})
...
async def synthesize_result(main_query: str, contents: list[str], max_context: int = 4000) -> str:
    trimmed_contents = [c[:1000] for c in contents if c]
    combined_content = " ".join(trimmed_contents)[:max_context]
    prompt = (
    ...
    try:
        while True:
            ...
            if chat_entry:
                past_messages = chat_entry.get("messages", [])
                if past_messages:
                    past_embeddings = np.array([msg["embedding"] for msg in past_messages if "embedding" in msg])
            ...
            generate_api_keys = [k for k in generate_api_keys if k]
            if not generate_api_keys:
                await websocket.send_json({"error": "No valid GROQ_API_KEY_GENERATE found"})
                continue
            selected_key = random.choice(generate_api_keys)
            client_generate = AsyncGroq(api_key=selected_key)
...
async def regenerate_response(request: RegenerateRequest, background_tasks: BackgroundTasks):
    try:
        user_id = request.user_id
        session_id = request.session_id
        filenames = request.filenames
        ...
        uploaded_files = await uploads_collection.distinct("filename", {"session_id": session_id})
        mentioned_filenames = [fn for fn in uploaded_files if fn.lower() in prompt.lower()]
        hooked_filenames = filenames if filenames else mentioned_filenames
        ...
        multimodal_context, used_filenames = await retrieve_multimodal_context(prompt, session_id, hooked_filenames)
...
async def analyze_images(
    ...
    response = await client.chat.completions.create(
        ...
        messages=[{"role": "system", "content": "Return response in plain english. Do not use LaTeX"},{"role": "user", "content": content}],
        temperature=1,
        max_completion_tokens=1024,
        top_p=1,
    )
    ...
    if updated_chat_entry and len(updated_chat_entry["messages"]) >= 10:
        await store_long_term_memory(user_id, session_id, updated_chat_entry["messages"][-10:])
...
async def goal_setting_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            conversation_history.append({"role": "assistant", "content": json.dumps(plan)})
        ...
        await goals_collection.insert_one(new_goal)
        await websocket.send_json({"message": "Great! Your goal has been saved. You can track your progress in the goals section."})
...
async def ai_assist(input_data: UserInput):
    ...
    try:
        ...
        caption = await generate_caption(input_data.query, seed_keywords, trending_hashtags, seo_keywords)
        return {"caption": caption, "keywords": seed_keywords, "hashtags": trending_hashtags, "seo_keywords": seo_keywords}
    except Exception as e:
...
async def websocket_ai_assist(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            await websocket.send_text(json.dumps({"step": "Generating Final Caption..."}))
            caption = await generate_caption(query, seed_keywords, trending_hashtags, seo_keywords)
...
async def websocket_generate_post(websocket: WebSocket):
    await websocket.accept()
    await websocket.send_json({"status": "connected", "message": "Proof: The websocket_generate_post function is running."})
    ...
    try:
        ...
        logger.info(f"--- GENERATE-POST: Received post_option: {post_option_type} ---") # DEBUG LOG
        ...
        if not client:
            ...
            await websocket.send_json({"status": "error", "message": "Server configuration error: Could not connect to AI service."})
            return
        ...
        await websocket.send_json(
            {"status": "processing", "message": f"Post classified as {post_type}. Generating keywords..."}
        )
        ...
        trending_hashtags = await fetch_trending_hashtags_post(client, seed_keywords)
        ...
        html_code, caption, media, parsed_media = None, None, None, None
        ...
        if post_option_type == PostGenOptions.Text:
            ...
            caption = await generate_caption_post(
                client, prompt, seed_keywords, trending_hashtags, seo_keywords
            )
        ...
        await websocket.send_json({"status": "error", "message": "A critical error occurred while generating the post. Please check the server logs."})
    finally:
        await websocket.close()
